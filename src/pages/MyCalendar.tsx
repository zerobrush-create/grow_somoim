    ...
    <div className="min-h-screen bg-background">
      <div className="mx-auto max-w-md pb-10">
        <header className="sticky top-0 z-30 bg-background/95 backdrop-blur-md border-b border-border px-4 py-3 flex items-center gap-3">
          <button onClick={() => navigate(-1)} className="h-9 w-9 rounded-full hover:bg-muted flex items-center justify-center" aria-label={tr("뒤로")}><ArrowLeft className="h-5 w-5" /></button>
          <h1 className="text-base font-bold flex-1 flex items-center gap-1.5"><CalendarDays className="h-4 w-4 text-primary" />{tr("내 캘린더")}</h1>
        </header>
        ...
        <section className="px-4 pt-2">
          <h3 className="text-sm font-bold mb-2">{tr("다가오는 일정")}</h3>
          {isLoading ? <Skeleton className="h-20 w-full" /> :
           upcoming.length === 0 ? <p className="text-center text-sm text-muted-foreground py-10">{tr("예정된 일정이 없어요")}</p> :
               ...
               return (
                 <Link key={e.id} to={`/groups/${e.group_id}/events`} className="block rounded-xl border border-border p-3 hover:shadow-soft transition-smooth">
                   <div className="flex items-start justify-between gap-2">
                     <h4 className="text-sm font-bold">{tr(e.title)}</h4>
                     {status && <Badge variant="secondary" className={cn("text-[10px]", status === "going" ? "bg-primary-soft text-primary" : status === "maybe" ? "bg-amber-100 text-amber-700" : "bg-muted")}>{tr(status === "going" ? "참석" : status === "maybe" ? "미정" : "불참")}</Badge>}
                   </div>
                   <div className="flex flex-wrap gap-2 text-[11px] text-muted-foreground mt-1">
                     <span className="inline-flex items-center gap-1"><CalendarDays className="h-3 w-3" />{formatDateTime(e.starts_at, lang, { dateStyle: "medium", timeStyle: "short" })}</span>
                     {e.location && <span className="inline-flex items-center gap-1"><MapPin className="h-3 w-3" />{tr(e.location)}</span>}
