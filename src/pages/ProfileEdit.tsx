...
const ProfileEdit = () => {
  ...
  const save = useMutation({
    mutationFn: async () => {
      if (!user) throw new Error(t.common.confirm);
      const { error } = await supabase.from("profiles").update({
        name, bio, location, mbti, interests, avatar_url: avatarUrl,
      }).eq("id", user.id);
      if (error) throw error;
    },
    onSuccess: () => { toast({ title: t.profileEdit.saved }); qc.invalidateQueries({ queryKey: ["profile-edit", user?.id] }); navigate("/profile"); },
    onError: (e: Error) => toast({ title: t.profileEdit.saveFailed, description: e.message, variant: "destructive" }),
  });
  ...
  const onUpload = async (file: File) => {
    ...
    try {
      ...
      toast({ title: t.profileEdit.saveFailed, description: e.message, variant: "destructive" });
    } finally {
      setUploading(false);
    }
  };
    ...
    <div className="min-h-screen bg-background">
      <div className="mx-auto max-w-md pb-10">
        <header className="sticky top-0 z-30 bg-background/95 backdrop-blur-md border-b border-border px-4 py-3 flex items-center gap-3">
          ...
          <h1 className="text-base font-bold flex-1">{t.profileEdit.title}</h1>
        </header>
        ...
        <div className="p-4 space-y-5">
          <div className="flex justify-center">
            <button onClick={() => fileRef.current?.click()} className="relative" disabled={uploading}>
              <div className="h-24 w-24 rounded-full bg-muted overflow-hidden">
                {avatarUrl ? <img src={avatarUrl} alt="avatar" className="h-full w-full object-cover" /> : <div className="h-full w-full flex items-center justify-center text-muted-foreground">{t.profileEdit.noImage}</div>}
              </div>
          ...
          <div className="space-y-1.5"><Label htmlFor="name">{t.profileEdit.name}</Label><Input id="name" value={name} onChange={(e) => setName(e.target.value)} /></div>
          <div className="space-y-1.5"><Label htmlFor="bio">{t.profileEdit.bio}</Label><Textarea id="bio" value={bio} onChange={(e) => setBio(e.target.value)} rows={3} /></div>
          <div className="space-y-1.5"><Label htmlFor="loc">{t.profileEdit.location}</Label><Input id="loc" value={location} onChange={(e) => setLocation(e.target.value)} placeholder={t.profileEdit.locationPlaceholder} /></div>

          <div className="space-y-2">
            <Label>{t.profileEdit.mbti}</Label>
          ...
          <div className="space-y-2">
            <Label>{t.profileEdit.interests}</Label>
              ...
              {INTEREST_OPTIONS.map((i) => {
                ...
                return (
                  <button key={i} type="button" onClick={() => toggleInterest(i)}>
                    <Badge variant={active ? "default" : "outline"} className={active ? "bg-primary text-primary-foreground" : ""}>{key ? t.interests[key] : i}</Badge>
                  </button>
                );
              })}
            </div>
          ...
          <Button onClick={() => save.mutate()} disabled={save.isPending} className="w-full h-12 gradient-primary shadow-glow">
            {save.isPending ? t.profileEdit.saving : t.profileEdit.save}
          </Button>
        </div>
      </div>
    </div>
